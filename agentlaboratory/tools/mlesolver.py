...
class MLESolver:
    ...
    def initial_solve(self):
        ...
        self.best_codes = [
            (copy(init_code), self.best_score, init_return) for _ in range(1)
        ]
    ...
    def solve(self):
        ...
        if top_score > self.best_codes[-1][1]:
            ...
            self.best_codes.append(
                (copy(self.code_lines), copy(top_score), self.prev_code_ret)
            )
    ...
    def process_command(self, model_resp):
        ...
        with suppress_stdout():  # shhh
                    ...
                    if cmd.cmd_type == "CODE-edit":
                        ...
                        for _tries in range(GLOBAL_REPAIR_ATTEMPTS):
                            ...
                            if success:
                                ...
                                code_err = (
                                    f"Return from executing code: {cmd_return[2]}"
                                )
            ...
            return (
                "Command not supported, choose from existing commands",
                None,
                None,
                None,
                None,
    ...
    def history_str(self):
        ...
        for _hist in range(len(self.st_history)):
            hist_str += (
                f"-------- History ({len(self.st_history) - _hist} steps ago) -----\n"
            )
            hist_str += (
                f"Because of the following response: {self.st_history[_hist][0]}\n"
                if len(self.st_history[_hist][0]) > 0
                else ""
            )
    ...
    def feedback(self, code_return):
        ...
        if code_return is not None:
            ...
            if "[CODE EXECUTION ERROR]" in code_return:
                ...
                print(
                    f"@@@@ SUBMISSION: model score {grade_return}",
                    REWARD_MODEL_LLM=self.llm_str,
                )
